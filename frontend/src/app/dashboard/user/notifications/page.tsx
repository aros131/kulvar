...
export default function UserNotificationsPage() {
  ...
  const markAsRead = async (id: string) => {
    const token = localStorage.getItem("token");
    await fetch(`https://kulvar-qb7t.onrender.com/dashboard/notifications/user/${id}/read`, {
      method: "PATCH",
      headers: {
  ...
  const markAllAsRead = async () => {
    const token = localStorage.getItem("token");
    await fetch(`https://kulvar-qb7t.onrender.com/dashboard/notifications/user/mark-all-read`, {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${token}`,
      },
