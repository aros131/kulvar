...
export async function GET(req: NextRequest) {
  ...
  const base = process.env.PERSE_BACKEND_URL || process.env.NEXT_PUBLIC_API_URL;
  const path = process.env.PERSE_COACHES_PATH || '/coaches';

  if (!base) {
    return NextResponse.json(
      { message: 'Server env PERSE_BACKEND_URL veya NEXT_PUBLIC_API_URL eksik.' },
      { status: 500 }
    );
  }
