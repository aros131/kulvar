...
const createProgram = async (req, res) => {
  try {
    const { name, description, duration, difficulty, nutritionPlan, dailySchedule, fitnessGoal, priceCents, isTemplate } = req.body;
...
const getCoachPrograms = async (req, res) => {
  try {
    const coachId = req.user._id;
    const programs = await Program.find({ coachId, isTemplate: { $ne: true } })
...
// 🟢 Get a coach's own templates
const getCoachTemplates = async (req, res) => {
  try {
    const coachId = req.user._id;
    const templates = await Program.find({ coachId, isTemplate: true }).sort({ createdAt: -1 });
    res.json({ templates });
  } catch (error) {
...
// 🟢 Convert an existing program into a reusable template (leaves the original program untouched)
const convertProgramToTemplate = async (req, res) => {
  try {
    const { programId } = req.params;
    const originalProgram = await Program.findById(programId);

    if (!originalProgram) return res.status(404).json({ message: "Program not found" });
    if (originalProgram.coachId.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: "Bu programı şablona dönüştürme yetkiniz yok" });
    }

    const template = await Program.create({
      ...originalProgram.toObject(),
      _id: undefined,
      slug: undefined, // regenerated by pre-validate hook to avoid unique collision
      name: `${originalProgram.name} (Şablon)`,
      isTemplate: true,
      sourceProgramId: originalProgram._id,
      assignedClients: [],
      priceCents: null,
      progressTracking: [],
      feedback: [],
...
// 🟢 Create a new program from one of the coach's templates
const createProgramFromTemplate = async (req, res) => {
  try {
    const { templateId } = req.params;
    const template = await Program.findById(templateId);
    ...
    const program = await Program.create({
      ...
      _id: undefined,
      slug: undefined,
      name: template.name.replace(/\s*\(Şablon\)\s*$/, ''),
      isTemplate: false,
    ...
    res.status(201).json({ message: "Program şablondan oluşturuldu", program });
  } catch (error) {
    res.status(500).json({ message: "Error creating program from template", error: error.message });
  }
};
